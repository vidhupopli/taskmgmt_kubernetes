...
const corsOptions = {
  origin: '*', // or whatever your frontend's URL is
...
const extractAndVerifyToken = async (headers) => {
  ...
  const response = await axios.get(`http://${authServiceURI}/verify-token/` + token);
...
app.listen(8000, () => { console.log('tasks service started @ port 8000'); });
